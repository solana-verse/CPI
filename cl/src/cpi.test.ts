...
test("init cpi", async () => {
	...
	const counter_pubkey = Keypair.generate().publicKey;
	const cpi_pubkey = Keypair.generate().publicKey;
	svm.addProgramFromFile(counter_pubkey, "./basic.so");
	svm.addProgramFromFile(cpi_pubkey, "./cpi.so");
	...
	const contractIxs = new TransactionInstruction({
		keys: [
			...
			{ pubkey: counter_pubkey, isSigner: false, isWritable: false },
		],
		programId: cpi_pubkey,
		data: Buffer.from(""), 
	});
